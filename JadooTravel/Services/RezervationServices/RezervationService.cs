...
namespace JadooTravel.Services.RezervationServices
{
    public class RezervationService : IRezervationService
    {
        ...
        {
            var values = _mapper.Map<Rezervation>(createRezervationDto);
            await _rezervationCollection.InsertOneAsync(values);
        }
        ...
        public async Task<GetRezervationByIdDto> GetRezervationByIdDtoAsync(string id)
        {
            var value = await _rezervationCollection.Find(x => x.RezervationId == id).FirstOrDefaultAsync();
